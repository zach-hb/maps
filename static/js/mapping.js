        ...
        d3.json(url).then(function(response) {
            ...
            response.forEach(function(brewery) {
                ...
                if(lat&&long) {
                    ...
                    .bindPopup("<h2>"+brewery.name+"</h2>"+'<hr>'
                    +'<h3>'+brewery.city+'</h3>'+'<hr>'
                    +'<h3>'+brewery.address_1+'</h3>'+'<hr>'
                    +'<h3>'+brewery.website_url+'</h3>'));
                }
            });
            // add heatlayer and marker layers to layer groups from above
            heatLayerGroup.addLayer(L.heatLayer(heatArray, {
                radius:30,
                blur:10
            }));
...
let overlays = {
    'Heat Map': heatLayerGroup,
    'Marker Cluster': markerClusterGroup
};
